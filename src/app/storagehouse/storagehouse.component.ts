...
@Component({
  ...
  templateUrl: './storagehouse.component.html',
  styleUrl: './storagehouse.component.css',
  providers: [StorageService, MessageService],
})
export class StoragehouseComponent implements OnInit {
  showForm = false;
  newItem: any = {
    // live: '',
    itemname: '',
  ...
  ngOnInit(): void {
    // this.storageService.getAllStorageItems().subscribe((data) => {
    //   this.storageList = data;
    //   console.log('Storage List:', this.storageList);

    //   this.storageList2 = data.map((item) => ({
    //     ...item,
    //     transferQty: null,
    //     location: '',
    //   }));
    // });
    // console.log('Storage List:', this.storageList);
    // console.log('Storage List2:', this.storageList2);

    this.setView(this.selectedPeriod);
    this.getStorageByDate();
  ...
  addStorageItem() {
    ...
    this.storageService.addStorageItem(item).subscribe({
      next: (data) => {
        this.messageService.add({
          severity: 'success',
          summary: 'Added',
  ...
  transferItem(row: any) {
    ...
    this.storageService.transferLifeSmoothi(payload).subscribe({
      next: () => {
  ...
  getStorageByDate() {
    ...
    this.storageService.getStorageByDate(this.fromDate, this.toDate).subscribe(
      (data) => {
